...
import { Plus, X, Trash2, FileImage, Send, CheckCircle2, RotateCcw, Calendar, Clock, Eye, MessageSquare, ChevronDown } from 'lucide-react';
...
export default function MarketingContent() {
  ...
  const handleDelete = async (id: number) => {
    await contentApi.delete(id); setDeleting(null); setDetail(null); load();
  };
  ...
  const grouped = STATUS_ORDER.reduce((acc, s) => {
    acc[s] = pieces.filter(p => p.status === s);
    return acc;
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handleComment} disabled={sendingComment || !comment.trim()}
                    className="btn-primary px-3 py-2 flex-shrink-0" style={{ fontSize: '0.75rem' }}>
                    <Send size={13} />
                  </button>
                </div>
              </div>
