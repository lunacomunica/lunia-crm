...
import { Save, Check, BookOpen, Users, Palette, Link2, Image as ImageIcon, Type, LayoutGrid, MessageSquare, Hash, AlertTriangle, ExternalLink, Plus, X, Smile } from 'lucide-react';
...
interface PosData {
  ...
  branding_sensacoes: string; // JSON Sensacao[]
  // Visual
  ...
  // Verbal
  verbal_tom: string;
  verbal_jargoes: string;
  verbal_emojis: string;
  // Território proibido
...
function SensacoesEditor({ value, onChange, accent }: { value: string; onChange: (v: string) => void; accent: string }) {
  ...
  const inputStyle = (focused: boolean) => ({
    background: focused ? bg : 'rgba(255,255,255,0.025)',
    border: `1px solid ${focused ? ring : 'rgba(255,255,255,0.06)'}`,
    color: 'rgba(226,232,240,0.9)', outline: 'none',
    padding: '10px 14px', borderRadius: '12px', fontSize: '13px', width: '100%',
    transition: 'all 0.2s',
        ...
        {items.map((s, i) => (
          <div key={i} className="grid gap-2 items-center" style={{ gridTemplateColumns: '1fr 2fr 28px' }}>
            <input value={s.sensacao} onChange={e => set(i, 'sensacao', e.target.value)}
              ...
              onFocus={e => Object.assign(e.currentTarget.style, { borderColor: ring, background: bg })}
              onBlur={e => Object.assign(e.currentTarget.style, { borderColor: 'rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.025)' })}
            />
      ...
      <div className="p-4">
        <textarea value={value} onChange={e => onChange(e.target.value)}
          ...
          style={{
            ...
            resize: 'none', lineHeight: '1.7', color: 'rgba(226,232,240,0.8)', fontFamily: 'inherit',
          }}
    ...
    <div className="relative">
      ...
      {tab === 'marca' && (
        <div className="space-y-4">
          <Card num="01" title="Sobre a Marca" accent={ac.color}
            desc="O que ela faz, onde atua, qual o momento atual. Objetivo e factual.">
            <Field label="Descrição da marca" accent={ac.color} value={data.sobre} onChange={set('sobre')} rows={4}
          ...
          <Card num="02" title="História & Trajetória" accent={ac.color}
            desc="Busque o conflito e a virada — não uma linha do tempo.">
            <Field label="De onde vem? O que construiu?" accent={ac.color} value={data.historia} onChange={set('historia')} rows={5}
          ...
          <Card num="03" title="Promessa" accent={ac.color}
            desc="Específica o suficiente para eliminar qualquer concorrente genérico.">
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Versão completa" accent={ac.color} value={data.promessa_completa} onChange={set('promessa_completa')} rows={5}
                placeholder="Eu ajudo [quem] a sair de [problema] através de [método], para que [resultado transformador]." />
              <Field label="Versão curta — bio ou stories" accent={ac.color} value={data.promessa_curta} onChange={set('promessa_curta')} rows={5}
                placeholder="A mesma promessa em 1 frase. Se demorar mais de 5 segundos para entender, refaça." />
            </div>
          </Card>

          <Card num="04" title="Transformação" accent={ac.color}
            desc="O mecanismo que justifica o preço — e a prova de que funciona.">
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Narrativa de transformação" accent={ac.color} value={data.transformacao_narrativa} onChange={set('transformacao_narrativa')} rows={5}
              ...
              <Field label="Procedência — por que acreditar" accent={ac.color} value={data.transformacao_procedencia} onChange={set('transformacao_procedencia')} rows={5}
                placeholder="Por que essa marca tem autoridade para entregar isso?&#10;Dado real, conquista verificável, frase de impacto." />
            </div>
          </Card>

          <Card num="05" title="Diferenciais Incopiáveis" accent={ac.color}
              ...
              {(['diferencial_1','diferencial_2','diferencial_3'] as const).map((k, i) => (
                <div key={k} className="relative">
                  <span className="absolute top-3 left-4 text-3xl font-black leading-none select-none"
                    style={{ color: `${ac.color}18` }}>0{i+1}</span>
                  <textarea value={data[k]} onChange={e => set(k)(e.target.value)}
                    ...
                    style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)', resize: 'none', lineHeight: '1.6', color: 'rgba(226,232,240,0.9)', fontFamily: 'inherit' }}
                    onFocus={e => { e.currentTarget.style.borderColor = `${ac.color}40`; e.currentTarget.style.boxShadow = `0 0 0 3px ${ac.color}15`; }}
      ...
      {tab === 'publico' && (
        <div className="space-y-4">
          <Card num="01" title="Perfil do Cliente Ideal" accent={ac.color}
            desc="Quem é, quem definitivamente não é, e o retrato psicológico completo.">
            <Field label="Quem é" accent={ac.color} value={data.icp_quem_e} onChange={set('icp_quem_e')} rows={3}
              placeholder="Descreva quem é o cliente ideal: perfil, contexto, fase de vida ou negócio..." />
            <Field label="Quem não é" accent={ac.color} value={data.icp_quem_nao_e} onChange={set('icp_quem_nao_e')} rows={3}
              placeholder="Quem essa marca não atende? Seja específico — não é só 'todo mundo'." />
            <Field label="Descrição psicológica" accent={ac.color} value={data.icp_psicologico} onChange={set('icp_psicologico')} rows={4}
              placeholder="Como ele pensa? O que o move? Como toma decisões? Qual é o seu mundo interno?" />
          </Card>

          <Card num="02" title="Crenças" accent={ac.color}
            desc="O que já tentou e por que não acredita mais em certas soluções.">
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="O que já tentou" accent={ac.color} value={data.crencas_tentou} onChange={set('crencas_tentou')} rows={5}
                placeholder="Quais soluções, métodos ou promessas ele já tentou antes de chegar aqui?" />
              <Field label="O que não acredita mais" accent={ac.color} value={data.crencas_nao_acredita} onChange={set('crencas_nao_acredita')} rows={5}
                placeholder="O que ele desacreditou? Que tipo de discurso já não cola mais com ele?" />
            </div>
          </Card>

          <Card num="03" title="Desejos" accent={ac.color}
            desc="O que ele realmente quer — por baixo das palavras.">
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Desejos mais secretos" accent={ac.color} value={data.desejos_secretos} onChange={set('desejos_secretos')} rows={5}
                placeholder="O que ele quer mas não fala em voz alta?" />
              <Field label="Sonho aspiracional" accent={ac.color} value={data.desejos_aspiracional} onChange={set('desejos_aspiracional')} rows={5}
                placeholder="Qual é o cenário dos sonhos que ele deseja alcançar? Seja visual e específico." />
            </div>
          </Card>

          <Card num="04" title="Dores" accent={ac.color}
            desc="As feridas que ainda não curaram — e onde a vida trava.">
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Dores mais profundas" accent={ac.color} value={data.dores_profundas} onChange={set('dores_profundas')} rows={5}
                placeholder="Quais são as dores que ele ainda não conseguiu resolver?" />
              <Field label="Onde sente que está travando" accent={ac.color} value={data.dores_travando} onChange={set('dores_travando')} rows={5}
                placeholder="Em que parte da vida ou do negócio ele sente que não consegue avançar?" />
            </div>
          </Card>

          <Card num="05" title="Objeções" accent={ac.color}
            desc="As desculpas e os medos que impedem a decisão.">
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Desculpas para não comprar" accent={ac.color} value={data.objecoes_desculpas} onChange={set('objecoes_desculpas')} rows={5}
                placeholder="O que ele diz para justificar a inação?" />
              <Field label="Medos de tomar a decisão errada" accent={ac.color} value={data.objecoes_medos} onChange={set('objecoes_medos')} rows={5}
      ...
      {tab === 'branding' && (
        <div className="space-y-4">
          ...
          {/* Direção central */}
          <Card icon={Palette} title="Direção Central do Branding" accent={ac.color}
            desc="O norte criativo que guia todas as decisões visuais e verbais da marca.">
            <Field label="Direção" accent={ac.color} value={data.branding_direcao} onChange={set('branding_direcao')} rows={4}
            ...
            <div>
              ...
              <SensacoesEditor value={data.branding_sensacoes} onChange={set('branding_sensacoes')} accent={ac.color} />
            </div>
          </Card>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(167,139,250,0.12)' }}>
            ...
            <div className="p-4 space-y-4" style={{ background: 'linear-gradient(145deg,#0c0c22,#0e0e2a)' }}>

              {/* Identidade Visual — só link */}
              <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <LinkField label="Link do Drive" value={data.visual_identidade_link} onChange={set('visual_identidade_link')}
                  placeholder="https://drive.google.com/drive/folders/..." accent={ac.color} />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <ImageField label="Imagem" urlValue={data.visual_logo_url} onUrlChange={set('visual_logo_url')}
                    ...
                    textPlaceholder="Quando usar versão principal, alternativa, monocromática..." accent={ac.color} />
                </div>
                ...
                <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <ImageField label="Paleta" urlValue={data.visual_cores_url} onUrlChange={set('visual_cores_url')}
                    ...
                    textPlaceholder="Primária para CTAs, secundária para fundos, neutras para texto..." accent={ac.color} />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div className="flex items-center gap-2.5 mb-3">
                    <Type size={13} style={{ color: '#a78bfa' }} />
                  ...
                  <ImageField label="Imagem" urlValue={data.visual_tipografia_url} onUrlChange={set('visual_tipografia_url')}
                    ...
                    textPlaceholder="Título, subtítulo, corpo de texto, legendas — fonte e peso para cada um..." accent={ac.color} />
                </div>

                <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div className="flex items-center gap-2.5 mb-3">
                    <LayoutGrid size={13} style={{ color: '#a78bfa' }} />
                  ...
                  <LinkField label="Pasta Pinterest" value={data.visual_mood_pinterest} onChange={set('visual_mood_pinterest')}
                    placeholder="https://pinterest.com/..." accent={ac.color} />
                  <div className="mt-3">
                    <ImageField label="Prévia do feed referência" urlValue={data.visual_mood_url} onUrlChange={set('visual_mood_url')} accent={ac.color} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(167,139,250,0.12)' }}>
            ...
            <div className="p-4 space-y-4" style={{ background: 'linear-gradient(145deg,#0c0c22,#0e0e2a)' }}>
              <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <Field label="Tom de voz, ritmo, formalidade e humor" accent={ac.color}
                  value={data.verbal_tom} onChange={set('verbal_tom')} rows={5}
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <Field label="Repetições da marca" accent={ac.color}
                    value={data.verbal_jargoes} onChange={set('verbal_jargoes')} rows={5}
                    placeholder="Expressões, frases ou palavras que se repetem e se tornam assinatura da marca..." />
                </div>

                <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div className="flex items-center gap-2.5 mb-3">
                    <Smile size={13} style={{ color: '#a78bfa' }} />
                    <span className="text-xs font-semibold text-white">Emojis permitidos</span>
                  </div>
                  <EmojiChips value={data.verbal_emojis} onChange={set('verbal_emojis')} accent={ac.color} />
                </div>
              </div>
            </div>
          </div>
