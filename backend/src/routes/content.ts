...
router.put('/:id', (req, res) => {
  ...
  const { title, type, caption, media_url, scheduled_date, objective, status, batch_id, copy_text, media_files, copy_hook, copy_cta, post_references } = req.body;
  const newStatus = status && VALID_STATUSES.includes(status) ? status : existing.status;

  db.prepare(`UPDATE content_pieces SET title=?, type=?, caption=?, media_url=?, scheduled_date=?, objective=?, status=?, batch_id=?, copy_text=?, media_files=?, copy_hook=?, copy_cta=?, post_references=?, updated_at=datetime('now') WHERE id=? AND tenant_id=?`).run(
