...
router.patch('/:id/integration', (req, res) => {
  ...
  const existing = db.prepare('SELECT * FROM agency_clients WHERE id=? AND tenant_id=?').get(req.params.id, req.user.tenant_id) as any;
  if (!existing) return res.status(404).json({ error: 'Cliente não encontrado' });
  const { instagram_user_id, meta_ads_account_id, waba_phone_number_id, waba_token } = req.body;
  db.prepare("UPDATE agency_clients SET instagram_user_id=?, meta_ads_account_id=?, waba_phone_number_id=?, waba_token=?, updated_at=datetime('now') WHERE id=? AND tenant_id=?")
    .run(
      instagram_user_id ?? existing.instagram_user_id,
      meta_ads_account_id ?? existing.meta_ads_account_id,
      waba_phone_number_id !== undefined ? (waba_phone_number_id || null) : existing.waba_phone_number_id,
      waba_token !== undefined ? (waba_token || null) : existing.waba_token,
      req.params.id, req.user.tenant_id
    );
  res.json({ ok: true });
});
