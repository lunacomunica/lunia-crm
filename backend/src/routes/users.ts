...
router.get('/', (req, res) => {
  const users = db.prepare(`
    SELECT u.id, u.name, u.email, u.role, u.job_title, u.avatar, u.agency_client_id, u.created_at,
...
router.post('/', (req, res) => {
  ...
  const { name, email, password, role = 'manager', agency_client_id, job_title, avatar } = req.body;
  ...
  const r = db.prepare('INSERT INTO users (tenant_id, name, email, password_hash, role, agency_client_id, job_title, avatar) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(req.user.tenant_id, name, email, hash, role, agency_client_id || null, job_title || null, avatar || null);
  res.json({ id: r.lastInsertRowid, name, email, role, agency_client_id: agency_client_id || null, job_title: job_title || null, avatar: avatar || null });
});

router.put('/:id', (req, res) => {
  if (req.user.role !== 'owner') return res.status(403).json({ error: 'Apenas admins podem editar usuários' });
  const { name, email, role, password, agency_client_id, job_title, avatar } = req.body;
  ...
  db.prepare('UPDATE users SET name=?, email=?, role=?, password_hash=?, agency_client_id=?, job_title=?, avatar=? WHERE id=? AND tenant_id=?').run(newName, newEmail, newRole, newHash, newClientId, newJobTitle, newAvatar, req.params.id, req.user.tenant_id);
  res.json({ id: req.params.id, name: newName, email: newEmail, role: newRole, agency_client_id: newClientId, job_title: newJobTitle, avatar: newAvatar });
});
