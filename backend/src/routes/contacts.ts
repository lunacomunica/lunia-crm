...
router.get('/', (req, res) => {
  ...
  let query = `SELECT c.*, r.name as referred_by_name FROM contacts c LEFT JOIN contacts r ON c.referred_by_id = r.id WHERE c.tenant_id = ? AND c.agency_client_id IS NULL AND NOT EXISTS (SELECT 1 FROM conversations cv WHERE cv.contact_id = c.id AND cv.agency_client_id IS NOT NULL)`;
  ...
  const total = (db.prepare('SELECT COUNT(*) as count FROM contacts c WHERE c.tenant_id = ? AND c.agency_client_id IS NULL AND NOT EXISTS (SELECT 1 FROM conversations cv WHERE cv.contact_id = c.id AND cv.agency_client_id IS NOT NULL)').get(tid) as any).count;
